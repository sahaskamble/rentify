...
class _EditProfileScreenState extends ConsumerState<EditProfileScreen> {
  ...
  void initState() {
    ...
    final profile = ref.read(sellerProfileProvider).valueOrNull;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: const Text('Edit Profile', style: TextStyle(color: AppColors.ink))),
          ...
          child: Column(children: [
            GestureDetector(
              ...
              child: CircleAvatar(
                ...
                child: _avatar == null ? const Icon(Icons.camera_alt, color: AppColors.ink) : null,
              ),
            ),
            ...
            const SizedBox(height: 10),
            TextFormField(controller: _bio, maxLength: 500, maxLines: 4, decoration: const InputDecoration(labelText: 'Bio')),
            const SizedBox(height: 10),
            TextFormField(controller: _city, decoration: const InputDecoration(labelText: 'City')),
            const SizedBox(height: 10),
            TextFormField(controller: _upi, decoration: const InputDecoration(labelText: 'UPI ID')),
            const SizedBox(height: 20),
            SizedBox(
              width: double.infinity,
              child: ElevatedButton(onPressed: _saving ? null : _save, child: const Text('Save')),
            ),
          ]),
        ),
      ),
  ...
  Future<void> _save() async {
    ...
    try {
      final body = <String, dynamic>{'name': _name.text.trim(), 'phone': _phone.text.trim()};
      if (_avatar != null) {
        body['avatar'] = await http.MultipartFile.fromPath('avatar', _avatar!.path);
      }
      await service.pb.collection('users').update(user.id, body: body);

      final existing = await service.pb.collection('seller_profiles').getList(filter: "user = '${user.id}'", perPage: 1);
      final sellerBody = {
        'user': user.id,
      ...
      if (existing.items.isEmpty) {
        ...
        await service.pb.collection('seller_profiles').update(existing.items.first.id, body: sellerBody);
      }
